        ...
        <header className="sticky top-0 z-40 bg-white/60 backdrop-blur-sm border-b">
            <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <a href="/home" className="text-lg font-semibold tracking-tight">Singkawang</a>
                ...
                <nav className="hidden lg:flex items-center gap-6 text-sm">
                    {path.map((name, i) => {
                        return <a href={`/${name.toLowerCase()}`} key={i} className={`hover:bg-slate-200 p-2 rounded-2xl ${location === `/${name.toLowerCase()}`? "animate-bounce bg-slate-200" : "animate-none"}`}>{name}</a>
                    })}
                </nav>
